    ...
    if (label.equalsIgnoreCase("SilkySpawner") || label.equalsIgnoreCase("SS") || label.equalsIgnoreCase("Spawner")) {
      if (args.length == 0 || (args.length >= 1 && !Arg1.contains(args[0].toLowerCase()))) {
        if (sender instanceof Player) {
          sender.spigot().sendMessage(cBuilder("/" + label + " help").create());
        } else
        ...
        try {
          ...
          Methods.sendLog(Utils.AddColors(errorCode), Level.SEVERE, false);
        }
      }
    }
    return true;
...
abstract class CommandsAbstractClass extends Storage {
  ...
  protected String getDes() { return ValidateCfg("CommandsAssistant." + getName() + ".Description"); }
  
  protected String getUsg() { return ValidateCfg("CommandsAssistant." + getName() + ".Usage"); }
}