...
public class LoreCommand extends CommandsAbstractClass {
  ...
  @Override
  public String getName() { return "lore"; }
  ...
  public void executeCommand(Player player, String label, String[] args) {
    ...
    List<String> sLore = sMeta.getLore().stream().filter(line -> !line.equals(sMeta.getLore().get(sMeta.getLore().size() -
  ...
  private void sendMes(Player player, String sCmd) {
    String cmd = sCmd + (!sCmd.equalsIgnoreCase("add") ? sCmd.equalsIgnoreCase("remove") ? " [line]" : " [line] <lore>" : " <lore>");
    player.spigot().sendMessage(cBuilder(label + cmd).create());
  }
}
