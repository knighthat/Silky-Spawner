...
public class LoreMenu extends MenuManager {
  ...
  @Override
  public String getMenuName() { return getInv("LoreModificatiorMenu.Title"); }
  ...
  public void itemClicked(InventoryClickEvent e) {
    ...
    switch (slot) {
      ...
      default :
        if (!permConfirm(player, "menu.lore." + commands.get(slot).toLowerCase()) && !permConfirm(player, "menu.*") && !permConfirm(player,
          "menu.lore."
            + "*"))
          return;
        if (slot != 0) {
  ...
  public void setMenuItems() {
    for (int slot : items.keySet())
      setInvItem(items.get(slot), 1, getInv("LoreModificatiorMenu." + commands.get(slot) + "LoreButton"), null, slot);
    GoBackButton(8);
  }
}
