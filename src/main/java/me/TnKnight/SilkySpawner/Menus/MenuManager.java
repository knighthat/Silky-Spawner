...
public abstract class MenuManager extends Storage implements InventoryHolder {
  ...
  public static String getInv(String path) {
  ...
  protected void accessModification(Player player, boolean type, ConfirmType cType) {
    ...
    player.sendMessage(
      getMsg(type ? "SetName" : "SetLore").replace("%min%",
        ValidateCfg("MinimumChars")).replace("%max%",
          ValidateCfg("MaximumChars")));
  ...
  protected void YesNoButton(int yesSlot, int noSlot) {
    for (int slot : Arrays.asList(yesSlot, noSlot))
      setInvItem(Material.getMaterial(slot == yesSlot ? "LIME_WOOL" : "RED_WOOL"), 1, getInv(slot == yesSlot ? "YesButton" : "NoButton"),
        null,
        slot);
  }
