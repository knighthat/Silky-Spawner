...
public enum MobsList {
	...
	private static final List<String> list = Arrays.asList(values()).stream().map(mob -> mob.toString()).filter(mob -> MobChecking(mob))
	...
	public static String getMobName(EntityType mobType) {
		return mobCustomName.get(mobType.name().toUpperCase());
	}
}
